...
export class AllpartnersComponent implements OnInit, AfterViewInit {
    ...
    constructor(public http: HttpClient, public commService: CommonService, public partnerService: PartnersService, private router: Router, private actRouter: ActivatedRoute, private alertService: SweetAlertService, private _script: ScriptLoaderService, private fb: FormBuilder) {
        ...
        this.catImgPath = this.commService._hostName + 'uploads/partners/';

    }

    ngOnInit() {

        this.commService.setTitle("كافة الشركاء");
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["partners_view", "partners_delete"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.partners_view = res.partners_view === '1' ? true : false;
                    this.partners_delete = res.partners_delete === '1' ? true : false;
                    this.isDataAvailable = true;
                    if (this.partners_view === true) {
                        this.getAllData();
                    }
                }
            });
    ...
    getAllData() {
        ...
        this.actRouter.queryParams.subscribe(params => {
            ...
            this.partnerService.getPartnersCount().subscribe((data) => {
                ...
                if (data) {
                    ...
                    if (data.status === 401) {
                        ...
                        this.partnerService.getSearchPartnersCount(sortParam, typeParam, this.searchName).subscribe((data) => {
                            if (data) {
                                this.noData = false;
    ...
    getSomeCountries(start, aItemsPerPage, sort, type, searchName) {
        ...
        this.partnerService.getSomePartners(start, aItemsPerPage, sort, type, searchName).subscribe((data) => {
            if (data) {
    ...
    goToSearch(searchName) {
        if (searchName != '') {
            this.router.navigate(['/partners/all-partners'], { queryParams: { page: 1, sort: this.countriesStatus, type: this.sortDirection, searchName: searchName } });
        }
    }
                    ...
                    this.alertService.warning({
                        ...
                        if (result.value) {
                            ...
                            if (this.commService.items.length !== 0) {
                                ...
                                this.partnerService.deletePartner(this.commService.items).subscribe((data) => {
                                    if (data) {
                                        ...
                                        this.partnerService.getPartnersCount().subscribe((data) => {
                                            if (data) {
                                                ...
                                                this.router.navigate(['/partners/all-partners']);
    ...
    getSortType(sort) {
        ...
        this.router.navigate(['/partners/all-partners'], { queryParams: { page: 1, sort: this.countriesStatus, type: this.sortDirection, searchName: this.searchName } });
    ...
    ngAfterViewInit() {
        ...
        Helpers.loadStyles('app-all-partners',
            ['assets/own-style/pagination/style.css']);

        this._script.loadScripts('app-all-partners',
