...
@Component({
    selector: "all-correspondenceMails",
  templateUrl: "./all-correspondenceMails.component.html",
    encapsulation: ViewEncapsulation.None,
})
export class AllcorrespondenceMailsComponent implements OnInit, AfterViewInit {
    ...
    correspondence_mails_view: boolean;
    correspondence_mails_delete: boolean;
  ...
  constructor(public http: HttpClient, public commService: CommonService, public langService: CorrespondenceMailsService, private router: Router, private actRouter: ActivatedRoute, private alertService: SweetAlertService, private _script: ScriptLoaderService, private fb: FormBuilder) {
    ...
    ngOnInit() {
        ...
        this.commService.setTitle("كافة الايميلات");
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["correspondence_mails_view", "correspondence_mails_delete"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.correspondence_mails_view = res.correspondence_mails_view === '1' ? true : false;
                    this.correspondence_mails_delete = res.correspondence_mails_delete === '1' ? true : false;
                    this.isDataAvailable = true;
                    if (this.correspondence_mails_view === true) {
                        this.getAllData();
                    }
                }
            });

            this.langImg = this.commService._hostName + 'uploads/correspondence_mails/';
    ...
    goToSearch(searchName) {
        if (searchName != '') {
            this.router.navigate(['correspondenceMails/all-correspondenceMails'], { queryParams: { page: 1, sort: this.langStatus, type: this.sortDirection, searchName: searchName } });
        }
    }
    ...
    deleteLanguageConfirm(value) {
        ...
        if (value.deleteMultiple === 'delete') {
            ...
            if (this.commService.items.length !== 0) {
                ...
                if (this.correspondence_mails_delete === false) {
                                        ...
                                        this.langService.getLanguagesCount().subscribe((data) => {
                                            if (data) {
                                                ...
                                                this.alertService.success({
                                                    title: 'حذف ايميل المراسله',
                                                    text: 'تم حذف ايميل المراسله بنجاح'
                                                });

                                                this.languagesCount = data;

                                                this.router.navigate(['correspondenceMails/all-correspondenceMails']);
    ...
    getSortType(sort) {
        ...
        this.router.navigate(['correspondenceMails/all-correspondenceMails'], { queryParams: { page: 1, sort: this.langStatus, type: this.sortDirection, searchName: this.searchName } });

    }

    goToAddLanguage() {
      this.router.navigate(['/correspondenceMails/add-correspondenceMail']);
    }
    ...
    ngAfterViewInit() {
        ...
        Helpers.loadStyles('all-correspondenceMails',
            ['assets/own-style/pagination/style.css']);

        this._script.loadScripts('all-correspondenceMails',
