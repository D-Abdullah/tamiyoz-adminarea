...
export class AllServicesComponent implements OnInit, AfterViewInit {
    ...
    constructor(public http: HttpClient, public commService: CommonService, public serService: ServicesService, private router: Router, private actRouter: ActivatedRoute, private alertService: SweetAlertService, private _script: ScriptLoaderService, private fb: FormBuilder) {
        ...
        this.catImgPath = this.commService._hostName + 'uploads/services/';

    }

    ngOnInit() {

        this.commService.setTitle("كافة الخدمات");
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["services_view", "services_delete"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.services_view = res.services_view === '1' ? true : false;
                    this.services_delete = res.services_delete === '1' ? true : false;
                    this.isDataAvailable = true;
                    if (this.services_view === true) {
                        this.getAllData();
                    }
                }
            });
    ...
    getAllData() {
        ...
        this.actRouter.queryParams.subscribe(params => {
            ...
            this.serService.getServicesCount().subscribe((data) => {
                ...
                if (data) {
                    ...
                    if (data.status === 401) {
                        ...
                        this.serService.getSearchServicesCount(sortParam, typeParam, this.searchName).subscribe((data) => {
                            if (data) {
                                this.noData = false;
    ...
    getSomeServices(start, aItemsPerPage, sort, type, searchName) {
        ...
        this.serService.getSomeServices(start, aItemsPerPage, sort, type, searchName).subscribe((data) => {
            if (data) {
    ...
    goToSearch(searchName) {
        if (searchName != '') {
            this.router.navigate(['/services/all-services'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: searchName } });
        }
    }
                    ...
                    this.alertService.warning({
                        ...
                        if (result.value) {
                            ...
                            if (this.commService.items.length !== 0) {
                                ...
                                this.serService.deleteService(this.commService.items).subscribe((data) => {
                                    if (data) {
    ...
    getSortType(sort) {
        ...
        this.router.navigate(['/services/all-services'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: this.searchName } });
    ...
    ngAfterViewInit() {
        ...
        Helpers.loadStyles('app-all-services',
            ['assets/own-style/pagination/style.css']);

        this._script.loadScripts('app-all-services',
