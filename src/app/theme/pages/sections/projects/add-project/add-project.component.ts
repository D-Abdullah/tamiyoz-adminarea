...
export class AddProjectComponent implements OnInit, AfterViewInit {
    ...
    category_sortErrorMsg: string = 'يرجى إدخال ترتيب الخبر';
    categoryIDErrorMsg: string = 'يرجى إدخال قسم الخبر';

    page_title_lang_arErrorMsg: string = 'يرجى إدخال عنوان الخبر باللغة العربية';

    page_title_lang_enErrorMsg: string = 'يرجى إدخال عنوان الخبر باللغة الإنجليزية';
    m_summernote_arErrorMsg: string = 'يرجى إدخال محتوى الخبر باللغة العربية';
    m_summernote_enErrorMsg: string = 'يرجى إدخال محتوى الخبر باللغة الإنجليزية';
    fileErrorMsg: string = 'يرجى إدخال صورة الخبر  ';

    constructor(public http: HttpClient,
        public commService: CommonService,
        public proService: ProjectService,
                ...
                if (data) {
                    ...
                    if (this.action === 'add' && this.page_id === null) {
                        ...
                        this.alertService.success({
                            ...
                            text: "تم حفظ بيانات الخبر بنجاح."
                        });
                        ...
                        this.alertService.success({
                            ...
                            text: "تم تعديل بيانات الخبر بنجاح."
                        });
                    }
                }
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            this.actRouter.paramMap.subscribe((params: ParamMap) => {
                ...
                if (this.page_id !== null) {
                    this.commService.setTitle("تعديل بيانات الخبر");

                    this.edit_action = true;
                    this.action = 'edit';
                }
                else if (this.page_id === null) {
                    this.commService.setTitle("إضافة خبر جديد");
    ...
    ngAfterViewInit() {
        ...
        let roles = ["projects_add", "projects_edit"];
        let userInfo = { "user_id": this.user_id, "roles": roles };

        this.commService.getUserLevel(userInfo).subscribe((res) => {
            if (res) {

                this.projects_add = res.projects_add === '1' ? true : false;
                this.projects_edit = res.projects_edit === '1' ? true : false;

                if (this.projects_edit === true && this.page_id !== null) {
                ...
                Helpers.loadStyles('app-add-project', 'assets/own-style/summernote-editor/edit-summernote.css');

                this._script.loadScripts('app-add-project',
