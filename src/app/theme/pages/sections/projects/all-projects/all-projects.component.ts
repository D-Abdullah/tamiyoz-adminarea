...
export class AllProjectsComponent implements OnInit, AfterViewInit {
    ...
    constructor(public http: HttpClient, public commService: CommonService, public serService: ProjectService, private router: Router, private actRouter: ActivatedRoute, private alertService: SweetAlertService, private _script: ScriptLoaderService, private fb: FormBuilder) {
    ...
    ngOnInit() {
        ...
        this.commService.setTitle("كافة الاخبار");
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["projects_view", "projects_delete"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.projects_view = res.projects_view === '1' ? true : false;
                    this.projects_delete = res.projects_delete === '1' ? true : false;
                    this.isDataAvailable = true;
                    if (this.projects_view === true) {
                        this.getAllData();
                    }
                }
            });
    ...
    goToSearch(searchName) {
        if (searchName != '') {
            this.router.navigate(['/projects/all-projects'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: searchName } });
        }
    }
    ...
    getSortType(sort) {
        ...
        this.router.navigate(['/projects/all-projects'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: this.searchName } });
    ...
    ngAfterViewInit() {
        ...
        Helpers.loadStyles('app-all-projects',
            ['assets/own-style/pagination/style.css']);

        this._script.loadScripts('app-all-projects',
