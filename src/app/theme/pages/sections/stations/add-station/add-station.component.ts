...
export class AddStationComponent implements OnInit, AfterViewInit {
  recivedfile:any;
  path:any;
  ...
  images: any = [];
    ...
    // news_add: boolean;
    // news_edit: boolean;
    stations_add: boolean;
    stations_edit: boolean;
    langImgPath: string;
    ...
    category_sortErrorMsg: string = 'يرجى إدخال ترتيب المشروع';
    categoryIDErrorMsg: string = 'يرجى إدخال قسم المشروع';

    page_title_lang_arErrorMsg: string = 'يرجى إدخال عنوان المشروع باللغة العربية';

    page_title_lang_enErrorMsg: string = 'يرجى إدخال عنوان المشروع باللغة الإنجليزية';
    m_summernote_arErrorMsg: string = 'يرجى إدخال محتوى المشروع باللغة العربية';
    m_summernote_enErrorMsg: string = 'يرجى إدخال محتوى المشروع باللغة الإنجليزية';
    fileErrorMsg: string = 'يرجى إدخال صور المشروع  ';
    uploadeFileErrorMsg:string='يرجى إدخال ملف المشروع '
    downloadFileFlage=false;
 filesImages=[];
    constructor(public http: HttpClient,
        public commService: CommonService,
        public proService: StationService,
    ...
    //   console.log("done");
      ...
      if (this.images.length >= 2) {
        ...
        if (this.page_title_lang_ar === '' || this.page_title_lang_en === '' || isEmptyDescription_ar === true || isEmptyDescription_en === true || this.success_file === true  || this.successfull_category_sort === true ) {
            ...
            this.proService.addEditStation(formData).subscribe((data) => {
                ...
                if (data) {
                    ...
                    if (this.action === 'add' && this.page_id === null) {
                        ...
                        this.alertService.success({
                            ...
                            text: "تم حفظ بيانات المشروع بنجاح."
                        });
  ...
  resetInputs() {
    ...
    getOneTrainingCourses(id,pagename) {
        ...
        this.proService.getOneStation(id,pagename).subscribe((data:any) => {
            if (data) {

             this.path =this.commService._hostName;
              ...
              if (data.images.length>0){
                ...
                if (data.stationfile === null || data.stationfile === '') {
                  ...
                  this.recivedfile=this.path + 'uploads/projects/files/' + data.stationfile;
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            this.actRouter.paramMap.subscribe((params: ParamMap) => {
                ...
                if (this.page_id !== null) {
                    this.commService.setTitle("تعديل بيانات المشروع");

                    this.edit_action = true;
                    this.action = 'edit';
                }
                else if (this.page_id === null) {
                    this.commService.setTitle("إضافة مشروع جديدة");
  ...
  addMoreImage(file) {
    ...
    ngAfterViewInit() {
        ...
        let roles = ["stations_add", "stations_edit"];
        let userInfo = { "user_id": this.user_id, "roles": roles };

        this.commService.getUserLevel(userInfo).subscribe((res) => {
            if (res) {

                this.stations_add = res.stations_add === '1' ? true : false;
                this.stations_edit = res.stations_edit === '1' ? true : false;

                if (this.stations_edit === true && this.page_id !== null) {
                ...
                Helpers.loadStyles('app-add-station', 'assets/own-style/summernote-editor/edit-summernote.css');

                this._script.loadScripts('app-add-station',
