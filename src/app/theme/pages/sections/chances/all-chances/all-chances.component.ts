...
@Component({
    selector: "app-all-chances",
    templateUrl: "./all-chances.component.html",
    encapsulation: ViewEncapsulation.None,
})
export class AllChancesComponent implements OnInit, AfterViewInit {
    ...
    chances_view: boolean;
    chances_delete: boolean;
    ...
    constructor(public http: HttpClient, public commService: CommonService, public serService: ChancesService, private router: Router, private actRouter: ActivatedRoute, private alertService: SweetAlertService, private _script: ScriptLoaderService, private fb: FormBuilder) {
        ...
        this.catImgPath = this.commService._hostName + 'uploads/chances/';

    }

    ngOnInit() {

        this.commService.setTitle("كافة الفرص");
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["chances_view", "chances_delete"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.chances_view = res.chances_view === '1' ? true : false;
                    this.chances_delete = res.chances_delete === '1' ? true : false;
                    this.isDataAvailable = true;
                    if (this.chances_view === true) {
                        this.getAllData();
                    }
                }
            });
    ...
    getAllData() {
        ...
        this.actRouter.queryParams.subscribe(params => {
            ...
            this.serService.getChancesCount().subscribe((data) => {
                ...
                if (data) {
                    ...
                    if (data.status === 401) {
                        ...
                        this.serService.getSearchChancesCount(sortParam, typeParam, this.searchName).subscribe((data) => {
                            if (data) {
                                this.noData = false;
    ...
    getSomeServices(start, aItemsPerPage, sort, type, searchName) {
        ...
        this.serService.getSomeChances(start, aItemsPerPage, sort, type, searchName).subscribe((data) => {
            if (data) {
    ...
    goToSearch(searchName) {
        if (searchName != '') {
            this.router.navigate(['/chances/all-chances'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: searchName } });
        }
    }
                    ...
                    this.alertService.warning({
                        ...
                        if (result.value) {
                            ...
                            if (this.commService.items.length !== 0) {
                                ...
                                this.serService.deleteChance(this.commService.items).subscribe((data) => {
                                    if (data) {
                                        ...
                                        this.serService.getChancesCount().subscribe((data) => {
                                            if (data) {
                                                ...
                                                this.alertService.success({
                                                    title: 'حذف الفرصه',
                                                    text: 'تم حذف الفرصه بنجاح'
                                                });

                                                this.servicesCount = data;

                                                this.router.navigate(['/chances/all-chances']);
    ...
    getSortType(sort) {
        ...
        this.router.navigate(['/chances/all-chances'], { queryParams: { page: 1, sort: this.servicesStatus, type: this.sortDirection, searchName: this.searchName } });

    }

    goToAddservice() {
        this.router.navigate(['/chances/chance']);
    }
    ...
    pagination(aTotal, page, sort, type, aItemsPerPage = 30, aLinksPerPage = 20) {
        ...
        this.getSomeServices(start, aItemsPerPage, sort, type, this.searchName);
    ...
    ngAfterViewInit() {
        ...
        Helpers.loadStyles('app-all-chances',
            ['assets/own-style/pagination/style.css']);

        this._script.loadScripts('app-all-chances',
