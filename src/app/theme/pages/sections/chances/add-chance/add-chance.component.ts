...
@Component({
    selector: "app-add-chance",
    templateUrl: "./add-chance.component.html",
    // providers: [ToolbarService, LinkService, ImageService, HtmlEditorService, TableService],
    encapsulation: ViewEncapsulation.None,
})
export class AddChanceComponent implements OnInit, AfterViewInit {
    public tools: object = {
        items: [
    ...
    category_sortErrorMsg: string = 'يرجى إدخال ترتيب الفرصه';
    categoryIDErrorMsg: string = 'يرجى إدخال قسم الفرصه';

    page_title_lang_arErrorMsg: string = 'يرجى إدخال عنوان الفرصه باللغة العربية';

    page_title_lang_enErrorMsg: string = 'يرجى إدخال عنوان الفرصه باللغة الإنجليزية';
    m_summernote_arErrorMsg: string = 'يرجى إدخال محتوى الفرصه باللغة العربية';
    m_summernote_enErrorMsg: string = 'يرجى إدخال محتوى الفرصه باللغة الإنجليزية';
    fileErrorMsg: string = 'يرجى إدخال صورة الفرصه  ';

    constructor(public http: HttpClient,
        public commService: CommonService,
        public serService: ChancesService,
    ...
    //   console.log("done");
        ...
        if (this.page_title_lang_ar === '' || this.page_title_lang_en === '' || isEmptyDescription_ar === true || isEmptyDescription_en === true || this.success_file === true || this.successfull_category_sort === true ) {
            ...
            this.serService.addEditChance(formData).subscribe((data) => {
                ...
                if (data) {
                    ...
                    if (this.action === 'add' && this.page_id === null) {
                        ...
                        this.alertService.success({
                            title: 'حفظ بيانات الفرصه',
                            text: "تم حفظ بيانات الفرصه بنجاح."
                        });

                    }
                    else {
                        this.alertService.success({
                            title: 'تعديل بيانات الفرصه',
                            text: "تم تعديل بيانات الفرصه بنجاح."
                        });
                    }
                }
    ...
    getOneTrainingCourses(id) {
        ...
        this.serService.getOneChance(id).subscribe((data) => {
            if (data) {
                ...
                if (data.img === null || data.img === '') {
                    ...
                    this.localUrl = this.commService._hostName + 'uploads/chances/' + data.img;
                }
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            this.actRouter.paramMap.subscribe((params: ParamMap) => {
                ...
                if (this.page_id !== null) {
                    this.commService.setTitle("تعديل بيانات الفرصه");

                    this.edit_action = true;
                    this.action = 'edit';
                }
                else if (this.page_id === null) {
                    this.commService.setTitle("إضافة فرصه جديدة");
    ...
    ngAfterViewInit() {
        ...
        let roles = ["chances_add", "chances_edit"];
        let userInfo = { "user_id": this.user_id, "roles": roles };

        this.commService.getUserLevel(userInfo).subscribe((res) => {
            if (res) {

                this.chances_add = res.chances_add === '1' ? true : false;
                this.chances_edit = res.chances_edit === '1' ? true : false;

                if (this.chances_edit === true && this.page_id !== null) {
                ...
                Helpers.loadStyles('app-add-chance', 'assets/own-style/summernote-editor/edit-summernote.css');

                this._script.loadScripts('app-add-chance',
