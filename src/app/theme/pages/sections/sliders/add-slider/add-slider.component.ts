...
export class AddSliderComponent implements OnInit {
    ...
    sliders_add: boolean;
    sliders_edit: boolean;
    status: boolean = true;
    sort: string = '0';
    successfull_country_inputs: any = [];
    fileErrorMsg: string = 'يرجى إدخال صورة السلايد';
    requiredInputsErrorMsg: string = 'يرجى إدخال عنوان السلايد';
    category_sortErrorMsg: string = 'يرجى إدخال ترتيب السلايد';
    SliderUrl:string='';
    myArrayLangs = [];
    ...
    constructor(public commService: CommonService, public sliderservice: slidersService, private fb: FormBuilder, private alertService: SweetAlertService, private router: Router, private actRouter: ActivatedRoute, private _script: ScriptLoaderService) {
      ...
      for (let err of this.successfull_country_inputs) {
        ...
        if (this.localUrl === this.defaultImg) {
            ...
            formData.append('status', this.status.toString());
             ...
             formData.append('SliderUrl', this.SliderUrl);
            ...
            if (this.action === 'add' && this.page_id === null) {
                ...
                this.sliderservice.addEditSlider(formData).subscribe((data) => {
                    ...
                    if (data) {
                        ...
                        this.alertService.success({
                            title: 'حفظ السلايد',
                            text: "تم حفظ بيانات السلايد بنجاح."
                        });
                    ...
                    if (data) {
                        ...
                        if (data === 403) {
                            this.alertService.error({
                                title: 'خطأ !!!',
                                text: "هذا السلايد غير موجودة !!"
                            });
                        }
                        else {
                            this.alertService.success({
                                title: 'تعديل السلايد',
                                text: "تم تعديل بيانات السلايد بنجاح."
                            });
                        }
                    }
    ...
    resetInputs() {
        ...
        for (let lang in this.myArrayLangs) {
            ...
            this.myArrayLangs[lang]['slider_des'] = '';
            this.myArrayLangs[lang]['slider_des_2'] = '';
        }
    ...
    getOneCountry(id) {
        ...
        this.sliderservice.getOneSlider(country_id).subscribe((data) => {
            if (data) {
                ...
                if (data.img === null || data.img === '') {
                    ...
                    this.localUrl = this.commService._hostName + 'uploads/sliders/' + data.img;
                }
                ...
                if(data.url=='undefined'){
                  ...
                  this.SliderUrl=data.url;
                }

                for (let lang in data['langs']) {
                    this.myArrayLangs[lang]['country_name'] = data['langs'][lang]['name'];
                    this.myArrayLangs[lang]['slider_des'] = data['langs'][lang]['description'];
                    this.myArrayLangs[lang]['slider_des_2'] = data['langs'][lang]['description_2'];
                }
    ...
    ngOnInit() {
        ...
        if (localStorage.getItem('currentUser') === null) {
            ...
            let roles = ["sliders_add", "sliders_edit"];
            let userInfo = { "user_id": this.user_id, "roles": roles };

            this.commService.getUserLevel(userInfo).subscribe((res) => {
                if (res) {
                    this.sliders_add = res.sliders_add === '1' ? true : false;
                    this.sliders_edit = res.sliders_edit === '1' ? true : false;

                    this.actRouter.paramMap.subscribe((params: ParamMap) => {
                        this.page_id = params.get('id');

                        if (this.page_id !== null) {
                            this.commService.setTitle("تعديل بيانات السلايد");

                            this.edit_action = true;
                            this.action = 'edit';

                            if (this.sliders_edit === true) {
